...
export function ForensicCaseTwoPanel({
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-[1.1fr_0.9fr] gap-5 items-start">
      ...
      <div className="rounded-lg border border-[#2A2D34] bg-[#16181D] p-5 select-text font-mono relative shadow-md">
        ...
        <div className="border-b border-[#2A2D34] pb-1.5 mb-3 mt-4 text-[11px] font-bold text-[#94A3B8] uppercase tracking-wider">
          AUTHENTICATION
        </div>
        <div className="grid grid-cols-3 gap-2.5 mb-1">
          {evidenceCardData.checks.map((c, idx) => (
            ...
            >
              <div className="text-[10px] text-[#8C94A0] font-semibold uppercase tracking-wider">
                {c.label}
              </div>
              <div
        ...
        {/* Section 2: ORIGIN & RELAY */}
        <div className="border-b border-[#2A2D34] pb-1.5 mb-3 mt-4 text-[11px] font-bold text-[#94A3B8] uppercase tracking-wider">
          {evidenceCardData.origin?.sectionTitle || 'ORIGIN & RELAY'}
        </div>
        <div className="grid grid-cols-[110px_1fr] gap-y-1.5 items-start text-xs font-mono py-1">
        ...
        <div className="border-b border-[#2A2D34] pb-1.5 mb-3 mt-4 text-[11px] font-bold text-[#94A3B8] uppercase tracking-wider">
          {evidenceCardData.entity?.sectionTitle || 'DOMAIN INTELLIGENCE'}
        </div>
        <div className="grid grid-cols-[110px_1fr] gap-y-1.5 items-start text-xs font-mono py-1">
      ...
      <div className="space-y-4">
        ...
        {evidenceCardData.aiSummary && (
          <div className="rounded-lg border border-[#2A2D34] bg-[#16181D] p-4 font-mono select-text shadow-sm">
            <div className="border-b border-[#2A2D34] pb-1.5 mb-3 text-[11px] font-bold text-[#94A3B8] uppercase tracking-wider">
              AI CASE SUMMARY
            </div>
            <p className="text-xs text-[#E7E4DA] leading-relaxed font-sans">
              {evidenceCardData.aiSummary.text}
            </p>
            <div className="flex items-center justify-between text-[11px] text-[#8C94A0] mt-3 pt-2 border-t border-[#2A2D34]/80">
              <span className="font-mono">{evidenceCardData.aiSummary.engine}</span>
              {onNavigateToLogs && (
                <button
                  onClick={onNavigateToLogs}
                  className="text-[#C68A34] hover:text-[#FBBF24] font-bold text-[11px] cursor-pointer"
                >
                  Full narrative ↗
                </button>
              )}
          ...
          <div className="rounded-lg border border-[#2A2D34] bg-[#16181D] p-4 font-mono select-text shadow-sm">
            ...
            <div className="space-y-2">
              {evidenceCardData.findings.map((f, idx) => (
                <div
                  key={idx}
                  className="flex items-center justify-between gap-3 text-xs py-1 border-b border-[#2A2D34]/60 last:border-0"
                >
                  <span className="text-[#F1EFEA] break-all font-mono">
                    {f.label}
                  </span>
                  <span
                    className={`px-2 py-0.5 rounded text-[10px] font-bold font-mono border shrink-0 ${
                      f.status === 'clean'
                        ? 'border-[#2E8B63] text-[#34D399] bg-[#2E8B63]/20'
                        : 'border-[#C6402F] text-[#F87171] bg-[#C6402F]/20'
                    }`}
                  >
                    {f.badge}
                  </span>
        ...
        {evidenceCardData.score && (
          <div className="rounded-lg border border-[#2A2D34] bg-[#16181D] p-4 font-mono select-text shadow-sm">
            ...
            <div className="flex items-center justify-between text-xs">
              <span className="text-[#8C94A0] font-medium">
                {evidenceCardData.score.label}
              </span>
              <span
                className={`font-bold ${
                  evidenceCardData.score.good ? 'text-[#34D399]' : 'text-[#F87171]'
                }`}
              >
                {evidenceCardData.score.resultText} {evidenceCardData.score.resultLabel}
              </span>
            </div>
            <div className="w-full h-2 rounded bg-[#1D2027] border border-[#2A2D34] overflow-hidden mt-2">
              <div
                className={`h-full rounded ${
                  evidenceCardData.score.good
                    ? 'bg-[#34D399]'
                    : 'bg-gradient-to-r from-[#C68A34] to-[#C6402F]'
                }`}
                style={{
