"""Inference for the trained BEC category score model."""
import os
import re
import json
import math
...
def predict_bec_category_scores(text: str) -> Optional[Dict[str, float]]:
    """
    Returns a trained probability score (0.0-1.0) per BEC category — replacing static
    hardcoded rule scores. Falls back to None if weights aren't trained yet.
    """
    data = _load()
    if not data:
        return None
    ...
    for cat in categories:
        coef = data["weights"][cat]
        bias = data["biases"][cat]
        logit = bias + sum(coef[idx] * v for idx, v in normalized.items() if idx < len(coef))
        results[cat] = round(1.0 / (1.0 + math.exp(-logit)), 4)  # sigmoid
    return results
